...
export async function api(path, { method = 'GET', body, signal, blob = false } = {}) {
  ...
  const response = await fetch(`${base}${path}`, { method, signal, headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}), ...(body !== undefined ? { 'Content-Type': 'application/json' } : {}) }, ...(body !== undefined ? { body: JSON.stringify(body) } : {}) });
  if (!response.ok) {
