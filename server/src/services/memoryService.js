...
export const assertSnapWindow = async (eventId, session) => {
  const settings = await SnapSettings.findOne({ eventId }).session(
    session || null,
  );
  if (windowStatus(settings) !== "OPEN")
    throw new ApiError(409, "Snap uploads are outside the configured window");
  return settings;
};
    ...
    return await mongoose.connection.transaction(async (session) => {
      ...
      if (slot === 2) {
        if (!privilegeCode)
          throw new ApiError(
            409,
            "A valid eligible pre-order code is required for a second photo",
          );
      ...
      const [memory] = await Memory.create(
        [{ userId, eventId, assetId: asset._id, caption, slot }],
        { session },
      );
...
export const deleteSnap = async ({ id, user, admin = false }) =>
  mongoose.connection.transaction(async (session) => {
    const filter = {
      _id: id,
      status: {
        $in: admin ? ["PENDING", "APPROVED"] : ["PENDING", "APPROVED"],
      },
    ...
    const update = {
      $set: {
        status: admin ? "ADMIN_REMOVED" : "OWNER_DELETED",
        deletedBy: user._id,
        deletedAt: new Date(),
      },
    };
    const deleted = await Memory.findOneAndUpdate(filter, update, {
      new: true,
      session,
    });
...
export const setReaction = async ({ memoryId, userId, reaction }) =>
  mongoose.connection.transaction(async (session) => {
    // Serialize with removal so reactions cannot be inserted after a photo is removed.
    const memory = await Memory.findOneAndUpdate(
      { _id: memoryId, status: "APPROVED" },
      { $inc: { reactionVersion: 1 } },
      { session, new: true },
    );
    if (!memory) throw new ApiError(404, "Photo not found");
    if (reaction === null)
      await MemoryReaction.deleteOne({ memoryId, userId }, { session });
    else
      await MemoryReaction.findOneAndUpdate(
        { memoryId, userId },
        { $set: { reaction } },
...
export const currentSnapContext = async (userId, testMode = false) => {
  ...
  if (userId) {
    ...
    result.used = await Memory.countDocuments({
      eventId: event._id,
      userId,
      slot: { $in: [1, 2] },
    });
