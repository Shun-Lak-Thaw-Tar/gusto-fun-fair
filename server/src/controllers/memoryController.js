...
const settingsSchema = z
  ...
  .refine((value) => new Date(value.opensAt) < new Date(value.closesAt), {
    message: "Opening must be before closing",
  });
  ...
  try {
    ...
    res.status(201).json({
      memory: {
        ...
        imageUrl: `/api/memories/${memory._id}/image`,
...
export const listMemories = async (req, res) => {
  ...
  const counts = await MemoryReaction.aggregate([
    { $match: { memoryId: { $in: page.map((row) => row._id) } } },
    {
      $group: {
        _id: { memoryId: "$memoryId", reaction: "$reaction" },
        count: { $sum: 1 },
      },
    },
  ]);
  ...
  res.json({
    memories: page.map((row) => ({
      id: row._id,
      caption: row.caption,
      imageUrl: `/api/memories/${row._id}/image`,
      createdAt: row.createdAt,
      ...(totals.get(String(row._id)) || { likes: 0, dislikes: 0 }),
    })),
...
export const getAdminMemoryImage = async (req, res) => {
  const memory = await Memory.findOne({
    _id: req.params.id,
    status: { $in: ["PENDING", "APPROVED"] },
  });
  if (!memory) throw new ApiError(404, "Photo not found");
  await sendImage(memory.assetId, res, { publicGallery: false });
};
export const deleteMyMemory = async (req, res) => {
  await deleteSnap({ id: req.params.id, user: req.user });
  res.status(204).end();
};
...
export const getMyMemories = async (req, res) => {
  ...
  const memories = await Memory.find({
    eventId: event._id,
    userId: req.user._id,
    status: { $in: ["PENDING", "APPROVED", "REJECTED", "ADMIN_REMOVED"] },
  })
    .select("_id caption status createdAt")
    .lean();
  res.json({
    memories: memories.map((row) => ({
      ...row,
      imageUrl:
        row.status === "APPROVED" ? `/api/memories/${row._id}/image` : null,
    })),
  });
};
export const updateSnapWindow = async (req, res) => {
  ...
  if (!parsed.success)
    throw new ApiError(
      400,
      "Provide an opening and later closing timestamp, each including Z or a timezone offset",
    );
  const event = await getCurrentEvent();
  await SnapSettings.findOneAndUpdate(
    { eventId: event._id },
...
export const reactToMemory = async (req, res) => {
  const parsed = z
    .object({ reaction: z.enum(["LIKE", "DISLIKE"]).nullable() })
    .strict()
    .safeParse(req.body);
  if (!parsed.success)
...
export const getMyReaction = async (req, res) => {
  if (!(await Memory.exists({ _id: req.params.id, status: "APPROVED" })))
    throw new ApiError(404, "Photo not found");
  const item = await MemoryReaction.findOne({
    memoryId: req.params.id,
    userId: req.user._id,
  });
...
export const listAdminMemories = async (req, res) => {
  ...
  if (!["PENDING", "APPROVED", "REJECTED", "ADMIN_REMOVED"].includes(status))
    throw new ApiError(400, "Invalid memory status");
  const memories = await Memory.find({ status })
    .sort({ createdAt: 1 })
  ...
  res.json({
    memories: memories.map((memory) => ({
      ...
      user: memory.userId,
      userId: undefined,
      imageUrl: ["PENDING", "APPROVED"].includes(memory.status)
        ? `/api/memories/${memory._id}/image`
        : null,
    })),
  });
};

export const reviewMemory = async (req, res) => {
  const parsed = z
    .object({
  ...
  const result = await mongoose.connection.transaction(async (session) => {
    ...
    if (!memory)
      throw new ApiError(409, "Only pending memories can be reviewed");
    memory.status = parsed.data.decision;
    memory.reviewedBy = req.user._id;
    memory.reviewedAt = new Date();
    memory.rejectionReason =
      parsed.data.decision === "REJECTED" ? parsed.data.reason : undefined;
