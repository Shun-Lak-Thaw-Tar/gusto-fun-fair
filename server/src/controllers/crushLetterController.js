...
const presentPublicLetter = (letter) => ({
...
export const createCrushLetter = async (req, res) => {
  ...
  res.status(201).json({
    ...
    crushLetter: {
      id: String(crushLetter._id),
      recipientName: crushLetter.recipientName,
      createdAt: crushLetter.createdAt,
...
export const listCrushLetters = async (req, res) => {
  ...
  if (!parsed.success)
    throw new ApiError(400, "Invalid pagination", z.treeifyError(parsed.error));
