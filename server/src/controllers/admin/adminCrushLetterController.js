...
import { z } from "zod";
import CrushLetter, {
  CRUSH_LETTER_STATUSES,
} from "../../models/CrushLetter.js";
...
const reviewSchema = z
  ...
  .strict()
  .refine((value) => value.decision === "APPROVED" || value.reason, {
...
const assertId = (id) => {
  if (!mongoose.isObjectIdOrHexString(id))
    throw new ApiError(400, "Invalid Crush Letter ID");
};
...
const presentAdminLetter = (letter) => ({
  ...
  reviewedBy: letter.reviewedBy
    ? { id: String(letter.reviewedBy._id), name: letter.reviewedBy.name }
    : null,
  rejectionReason: letter.rejectionReason || null,
});
...
export const listCrushLetters = async (req, res) => {
  ...
  if (!parsed.success)
    throw new ApiError(
      400,
      "Invalid Crush Letter query",
      z.treeifyError(parsed.error),
    );
  const { status, page, limit } = parsed.data;
  const filter =
    status === "PENDING" ? pendingFilter : status ? { status } : {};
  const [letters, total] = await Promise.all([
    CrushLetter.find(filter)
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
  ...
  res.json({
    crushLetters: letters.map(presentAdminLetter),
...
export const getCrushLetter = async (req, res) => {
  ...
  const letter = await CrushLetter.findById(req.params.id)
    .populate("reviewedBy", "name")
    .lean();
  ...
  const letter = await CrushLetter.findOneAndUpdate(
    { _id: req.params.id, ...pendingFilter },
    {
      $set: {
        ...
        rejectionReason:
          parsed.data.decision === "REJECTED" ? parsed.data.reason : null,
      },
    },
    { new: true },
  ).populate("reviewedBy", "name");
  if (!letter) {
    if (!(await CrushLetter.exists({ _id: req.params.id })))
      throw new ApiError(404, "Crush Letter not found");
    throw new ApiError(409, "Only pending Crush Letters can be reviewed");
  }
  res.json({
    message: `Crush Letter ${parsed.data.decision.toLowerCase()}.`,
    crushLetter: presentAdminLetter(letter),
  });
};
...
export const updateCrushLetterVisibility = async (req, res) => {
  ...
  if (!letter) {
    ...
    throw new ApiError(
      409,
      parsed.data.hidden
        ? "Only approved Crush Letters can be hidden"
        : "Only hidden Crush Letters can be restored",
    );
  }
  res.json({
    message: parsed.data.hidden
      ? "Crush Letter hidden."
      : "Crush Letter restored.",
    crushLetter: presentAdminLetter(letter),
  });
};
