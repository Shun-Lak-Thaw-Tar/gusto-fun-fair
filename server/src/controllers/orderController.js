...
import {
  cancelOrder as cancelOrderReservation,
  declarePayment,
...
const requestedItemSchema = z
  ...
  .strict()
  .refine((item) => Boolean(item.stallFoodId) !== Boolean(item.foodItemId), {
    message:
      "Provide exactly one stallFoodId; foodItemId is deprecated compatibility input",
  });
const schema = z
  .object({ items: z.array(requestedItemSchema).min(1) })
  .strict();
export const createOrder = async (req, res) => {
  ...
  if (!parsed.success)
    throw new ApiError(400, "Invalid order data", z.treeifyError(parsed.error));
  const canonicalIds = parsed.data.items.flatMap((item) =>
    item.stallFoodId ? [item.stallFoodId] : [],
  );
  const legacyIds = parsed.data.items.flatMap((item) =>
    item.foodItemId ? [item.foodItemId] : [],
  );
  const entries = await StallFood.find({
    $or: [
      { _id: { $in: canonicalIds } },
      { legacyFoodItemId: { $in: legacyIds } },
    ],
  })
    .populate("stallId")
    .populate("foodId");
  ...
  const byLegacy = new Map(
    entries
      .filter((entry) => entry.legacyFoodItemId)
      .map((entry) => [String(entry.legacyFoodItemId), entry]),
  );
  const consolidatedMap = new Map();
  for (const requested of parsed.data.items) {
    const entry = requested.stallFoodId
      ? byCanonical.get(requested.stallFoodId)
      : byLegacy.get(requested.foodItemId);
    if (!entry) throw new ApiError(400, "One or more stall foods do not exist");
    const id = String(entry._id);
    consolidatedMap.set(id, {
      stallFoodId: id,
      quantity: (consolidatedMap.get(id)?.quantity || 0) + requested.quantity,
    });
  }
  ...
  try {
    ...
    const order = await Order.create({
      ...
      reservationExpiresAt: new Date(
        now.getTime() + config.orderReservationMinutes * 60_000,
      ),
    });
    res
      .status(201)
