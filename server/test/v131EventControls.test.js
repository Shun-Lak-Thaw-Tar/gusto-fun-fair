...
test('Backend V1.3.1 event schedule and feature controls', async (t) => {
  ...
  await mongoose.connect(uri, { dbName: `funfair_v131_test_${process.pid}` });
  ...
  await t.test('updating Event Page preserves Memories', async () => {
    ...
    assert.deepEqual(result.body.event.featureFlags, { memoriesEnabled: true, eventPageEnabled: true, crushLettersEnabled: false, quizEnabled: false });
  });
  await t.test('Admin independently disables Memories', async () => {
    ...
    assert.deepEqual(result.body.event.featureFlags, { memoriesEnabled: false, eventPageEnabled: true, crushLettersEnabled: false, quizEnabled: false });
  });
  await t.test('Admin independently disables Event Page', async () => {
    ...
    assert.deepEqual(result.body.event.featureFlags, { memoriesEnabled: false, eventPageEnabled: false, crushLettersEnabled: false, quizEnabled: false });
  });
