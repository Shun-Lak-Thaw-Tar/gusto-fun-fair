...
test('Backend V1.1 lifecycle', async (t) => {
  ...
  await FoodItem.create([{ _id: ids.foodA, stallId: ids.stallA, name: 'Burger', eventDayPrice: 5000, ticketLimit: 5, isAvailable: true }, { _id: ids.foodB, stallId: ids.stallB, name: 'Tea', eventDayPrice: 2500, ticketLimit: 3, isAvailable: true }, { _id: ids.foodC, stallId: ids.stallA, name: 'Unavailable', eventDayPrice: 1000, ticketLimit: 2, isAvailable: false }]);
  ...
  await t.test('order creation consolidates duplicates and reserves inventory', async () => {
    const res = response();
    await createOrder({ user, body: { items: [{ foodItemId: String(ids.foodA), quantity: 1 }, { foodItemId: String(ids.foodA), quantity: 2 }, { foodItemId: String(ids.foodB), quantity: 1 }] } }, res);
  ...
  await t.test('inventory counters reflect the reservation', async () => { const foods = await FoodItem.find({ _id: { $in: [ids.foodA, ids.foodB] } }).sort({ _id: 1 }); assert.deepEqual(foods.map((food) => food.reservedTickets), [3, 1]); });
  await t.test('insufficient inventory is rejected atomically', () => expectStatus(reserveInventory([{ foodItemId: ids.foodA, foodName: 'Burger', quantity: 3 }]), 409));
  await t.test('unavailable item cannot be reserved', () => expectStatus(reserveInventory([{ foodItemId: ids.foodC, foodName: 'Unavailable', quantity: 1 }]), 409));
  await t.test('concurrent reservations cannot oversell', async () => { const outcomes = await Promise.allSettled([reserveInventory([{ foodItemId: ids.foodB, quantity: 2 }]), reserveInventory([{ foodItemId: ids.foodB, quantity: 2 }])]); assert.equal(outcomes.filter((item) => item.status === 'fulfilled').length, 1); assert.equal((await FoodItem.findById(ids.foodB)).reservedTickets, 3); });
  await t.test('multi-item failure compensates earlier items', async () => { await releaseInventory([{ foodItemId: ids.foodB, quantity: 2 }]); const before = (await FoodItem.findById(ids.foodB)).reservedTickets; await expectStatus(reserveInventory([{ foodItemId: ids.foodB, quantity: 1 }, { foodItemId: ids.foodC, quantity: 3 }]), 409); assert.equal((await FoodItem.findById(ids.foodB)).reservedTickets, before); });
  await t.test('invalid quantity is rejected by order validation', () => expectStatus(createOrder({ user, body: { items: [{ foodItemId: String(ids.foodA), quantity: 0 }] } }, response()), 400));
  ...
  await t.test('admin approval converts reserved inventory to sold', async () => { const payment = await Payment.findOne({ orderId: created._id }); const before = await FoodItem.findById(ids.foodA); const result = await reviewPayment({ paymentId: payment._id, decision: 'APPROVED', adminId: admin._id }); const after = await FoodItem.findById(ids.foodA); assert.equal(result.order.status, 'PAYMENT_APPROVED'); assert.equal(after.reservedTickets, before.reservedTickets - 3); assert.equal(after.soldTickets, before.soldTickets + 3); assert.equal(ticketsRemaining(after), ticketsRemaining(before)); });
  await t.test('approval creates exactly one ticket', async () => assert.equal(await Ticket.countDocuments({ orderId: created._id }), 1));
  await t.test('repeated approval does not double-sell or duplicate ticket', async () => { const payment = await Payment.findOne({ orderId: created._id }); const before = await FoodItem.findById(ids.foodA); await reviewPayment({ paymentId: payment._id, decision: 'APPROVED', adminId: admin._id }); const after = await FoodItem.findById(ids.foodA); assert.equal(after.soldTickets, before.soldTickets); assert.equal(await Ticket.countDocuments({ orderId: created._id }), 1); assert.equal(await Notification.countDocuments({ orderId: created._id, type: 'PAYMENT_APPROVED' }), 1); });
  await t.test('approved order cannot be cancelled', () => expectStatus(cancelOrder(created), 409));

  let cancellable;
  await t.test('awaiting-payment owner can cancel and release once', async () => { await reserveInventory([{ foodItemId: ids.foodB, quantity: 1 }]); cancellable = await Order.create({ userId: user._id, items: [{ stallId: ids.stallB, foodItemId: ids.foodB, stallName: 'B', foodName: 'Tea', quantity: 1, unitPrice: 2000, subtotal: 2000 }], totalQuantity: 1, totalAmount: 2000, paymentReference: 'FF-ORDER-CANCEL', reservationExpiresAt: new Date(Date.now() + 10000) }); const before = (await FoodItem.findById(ids.foodB)).reservedTickets; cancellable = await cancelOrder(cancellable); assert.equal(cancellable.status, 'CANCELLED'); assert.equal(cancellable.inventoryStatus, 'RELEASED'); assert.equal((await FoodItem.findById(ids.foodB)).reservedTickets, before - 1); });
  await t.test('repeated cancellation cannot release twice', async () => { const before = (await FoodItem.findById(ids.foodB)).reservedTickets; await expectStatus(cancelOrder(cancellable), 409); assert.equal((await FoodItem.findById(ids.foodB)).reservedTickets, before); });
  ...
  let expiring;
  await t.test('awaiting-payment expiry releases inventory', async () => { await reserveInventory([{ foodItemId: ids.foodB, quantity: 1 }]); expiring = await Order.create({ userId: user._id, items: [{ stallId: ids.stallB, foodItemId: ids.foodB, stallName: 'B', foodName: 'Tea', quantity: 1, unitPrice: 2000, subtotal: 2000 }], totalQuantity: 1, totalAmount: 2000, paymentReference: 'FF-ORDER-EXPIRE', reservationExpiresAt: new Date(0) }); const before = (await FoodItem.findById(ids.foodB)).reservedTickets; await releaseExpiredReservations(); expiring = await Order.findById(expiring._id); assert.equal(expiring.status, 'EXPIRED'); assert.equal(expiring.inventoryStatus, 'RELEASED'); assert.equal((await FoodItem.findById(ids.foodB)).reservedTickets, before - 1); });
  await t.test('expired order cannot declare payment', () => expectStatus(declarePayment(expiring), 409));
  await t.test('repeated expiry cleanup does not double-release', async () => { const before = (await FoodItem.findById(ids.foodB)).reservedTickets; await releaseExpiredReservations(); assert.equal((await FoodItem.findById(ids.foodB)).reservedTickets, before); });

  let evidenceExpired;
  await t.test('evidence deadline expiry uses distinct status and releases', async () => { await reserveInventory([{ foodItemId: ids.foodB, quantity: 1 }]); evidenceExpired = await Order.create({ userId: user._id, items: [{ stallId: ids.stallB, foodItemId: ids.foodB, stallName: 'B', foodName: 'Tea', quantity: 1, unitPrice: 2000, subtotal: 2000 }], totalQuantity: 1, totalAmount: 2000, paymentReference: 'FF-ORDER-EVIDENCE', reservationExpiresAt: new Date(Date.now() + 10000), status: 'PAYMENT_DECLARED', paymentDeclaredAt: new Date(0), paymentProofExpiresAt: new Date(0) }); await releaseExpiredReservations(); evidenceExpired = await Order.findById(evidenceExpired._id); assert.equal(evidenceExpired.status, 'PAYMENT_EVIDENCE_EXPIRED'); assert.equal(evidenceExpired.inventoryStatus, 'RELEASED'); });
  ...
  let rejected;
  await t.test('admin rejection releases reserved inventory and creates no ticket', async () => { await reserveInventory([{ foodItemId: ids.foodB, quantity: 1 }]); rejected = await Order.create({ userId: user._id, items: [{ stallId: ids.stallB, foodItemId: ids.foodB, stallName: 'B', foodName: 'Tea', quantity: 1, unitPrice: 2000, subtotal: 2000 }], totalQuantity: 1, totalAmount: 2000, paymentReference: 'FF-ORDER-REJECT', reservationExpiresAt: new Date(Date.now() + 10000), status: 'PAYMENT_SUBMITTED' }); const payment = await Payment.create({ orderId: rejected._id, userId: user._id, paymentProof: { url: '/proof.jpg' } }); const result = await reviewPayment({ paymentId: payment._id, decision: 'REJECTED', rejectionReason: 'Unreadable proof', adminId: admin._id }); rejected = result.order; assert.equal(rejected.status, 'PAYMENT_REJECTED'); assert.equal(rejected.inventoryStatus, 'RELEASED'); assert.equal(await Ticket.countDocuments({ orderId: rejected._id }), 0); });
  await t.test('repeated rejection has no duplicate release or notification', async () => { const payment = await Payment.findOne({ orderId: rejected._id }); const before = (await FoodItem.findById(ids.foodB)).reservedTickets; await reviewPayment({ paymentId: payment._id, decision: 'REJECTED', rejectionReason: 'Unreadable proof', adminId: admin._id }); assert.equal((await FoodItem.findById(ids.foodB)).reservedTickets, before); assert.equal(await Notification.countDocuments({ orderId: rejected._id, type: 'PAYMENT_REJECTED' }), 1); });
