...
test('Backend V1.1 lifecycle', async (t) => {
  ...
  await StallFood.create([{ _id: ids.foodA, stallId: ids.stallA, foodId: ids.genericA, eventDayPrice: 5000, discount: { type: 'percentage', value: 10 }, ticketLimit: 6, isAvailable: true }, { _id: ids.foodB, stallId: ids.stallB, foodId: ids.genericB, eventDayPrice: 2500, discount: { type: 'fixed', value: 500 }, ticketLimit: 3, isAvailable: true }, { _id: ids.foodC, stallId: ids.stallA, foodId: ids.genericC, eventDayPrice: 1000, discount: { type: 'percentage', value: 10 }, ticketLimit: 2, isAvailable: false }]);
  ...
  await t.test('order creation consolidates duplicates and reserves inventory', async () => {
    const res = response();
    await createOrder({ user, body: { items: [{ stallFoodId: String(ids.foodA), quantity: 1 }, { stallFoodId: String(ids.foodA), quantity: 1 }, { stallFoodId: String(ids.foodB), quantity: 1 }] } }, res);
    created = res.body.order;
    assert.equal(res.statusCode, 201); assert.equal(created.items.length, 2); assert.equal(created.totalQuantity, 3); assert.equal(created.inventoryStatus, 'RESERVED');
  });
  await t.test('multi-stall order stores authoritative prices', () => { assert.deepEqual(created.items.map((item) => item.unitPrice).sort(), [2000, 4500]); assert.equal(created.totalAmount, 11000); });
  await t.test('reservation deadline caps existing 60-minute configuration at 30 minutes', () => assert.ok(Math.abs(created.reservationExpiresAt - created.createdAt - 1_800_000) < 1000));
  ...
  await t.test('inventory counters reflect the reservation', async () => { const foods = await StallFood.find({ _id: { $in: [ids.foodA, ids.foodB] } }).sort({ _id: 1 }); assert.deepEqual(foods.map((food) => food.reservedTickets), [2, 1]); });
  await t.test('insufficient inventory is rejected atomically', () => expectStatus(reserveInventory([{ stallFoodId: ids.foodA, foodName: 'Burger', quantity: 5 }]), 409));
  ...
  await t.test('admin approval converts reserved inventory to sold', async () => { const payment = await Payment.findOne({ orderId: created._id }); const before = await StallFood.findById(ids.foodA); const result = await reviewPayment({ paymentId: payment._id, decision: 'APPROVED', adminId: admin._id }); const after = await StallFood.findById(ids.foodA); assert.equal(result.order.status, 'PAYMENT_APPROVED'); assert.equal(after.reservedTickets, before.reservedTickets - 2); assert.equal(after.soldTickets, before.soldTickets + 2); assert.equal(ticketsRemaining(after), ticketsRemaining(before)); });
